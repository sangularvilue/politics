...
export async function POST(req: NextRequest) {
  ...
  const tok = token();
  await redis.set(resetKey(tok), { uid: user.id, exp: Date.now() + RESET_TTL * 1000 }, { ex: RESET_TTL });
  const link = `${baseUrl(req.url)}/reset/${tok}`;

  const result = await sendEmail({
    to: emailLower,
    subject: "Reset your password — Aristotle's Politics",
    text: `Reset your password for Aristotle's Politics using this link (valid for 1 hour):\n\n${link}\n\nIf you didn't request this, you can ignore this email.`,
    html: `<p>Reset your password for <strong>Aristotle's Politics</strong> using the link below (valid for 1 hour):</p>
<p><a href="${link}">${link}</a></p>
<p style="color:#888">If you didn't request this, you can safely ignore this email.</p>`,
  });

  if (!result.delivered && process.env.NODE_ENV !== "production") generic.devLink = link;
